...
def create_appointments_window():
    ...
    tk.Label(window, text="Formato: DD-MM-YYYY HH:MM", font=('Arial', 10)).pack(pady=5)
...
def create_payments_window():
    ...
    date_entry = tk.Entry(window, font=('Arial', 12))
    date_entry.pack(pady=5)
    tk.Label(window, text="Formato: DD-MM-YYYY", font=('Arial', 10)).pack(pady=5)
...
def create_progress_tracking_window():
    ...
    def save_progress():
        ...
        # Assuming progress data is saved in a JSON file
        save_data(progress, 'progress.json')  # This would append to a list or handle accordingly
...
def create_communication_window():
    ...
    def send_message():
        ...
        # In a real application, integrate with an email or messaging service
...
def create_reports_window():
    ...
    def generate_report():
        ...
        # Implement actual report generation logic based on the selected type
...
def open_personal_main_app(cpf):
    ...
    def view_clients():
        ...
        for cpf in clients:
            client_list.insert(tk.END, f"{clients[cpf]['name']} - CPF: {cpf} - Peso: {cpf} - Altura: {cpf}")
...
def register_personal():
    global register_window, user_type_var_register_personal
    register_window = tk.Toplevel(root)
    register_window.title("Registro")
    register_window.geometry("1080x1000")
    center_window(register_window, 1080, 1000)
...
def save_registration_personal():
    ...
    if not name or not password:
        ...
        return
    users = load_users()
    if name in users['personal']:
        messagebox.showerror("Erro", "Usuário já cadastrado")
    else:
        users['personal'][name] = {'Nome': name, 'Senha': password}
        save_users(users)
        messagebox.showinfo("Sucesso", "Cadastro realizado com sucesso")
        register_window.destroy()

def save_registration_client():
    user_type = user_type_var_register_client.get()
    name = name_register_entry.get()
    ...
    if cpf in users[user_type]:
        ...
        users[user_type][cpf] = {'Nome': name, 'CPF': cpf, 'Contato':contato, 'Sexo': genero, 'Peso': peso, 'Altura': altura, 'Senha': password}
        save_users(users)
        messagebox.showinfo("Sucesso", "Cadastro realizado com sucesso")
        register_window.destroy()
...
def login():
    ...
    if user_type in users:
        if nome in users[user_type] and users[user_type][nome]['Senha'] == password:
