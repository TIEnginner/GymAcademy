...
def create_appointments_window():
    ...
    clients = list(users['cliente'][nome]['Nome'].keys())
...
def create_reports_window():
    ...
    def generate_report():
        ...
        report_content = f"Relatório de {selected_report}\n\nAqui está o conteúdo do relatório de {selected_report}\n\n{message_text.get("1.0", tk.END).strip()}"
        ...
        if file_path:
            try:
                ...
                messagebox.showinfo("Sucesso", f"Relatório de '{selected_report}' salvo com sucesso!")
